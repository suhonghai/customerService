...
> 单包内单元 / 集成 spec **不放这里**。详见根 `CLAUDE.md`「Spec 落点分层」段。
...
详见根 `CLAUDE.md` 的「Spec 落点分层(3 个落点,各管各的)」段(包含 4 列表、核心原则、反模式段、代表性例子表)。本目录(`tests/_specs/`)只承担「跨包 / 端到端 / 用户可见行为」一种落点。

**核心原则**:**spec 不依赖任何特定子包的实现细节** → 本目录;否则 co-located。
