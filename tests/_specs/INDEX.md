...
| cs-round-028                                                  | 架构层消除 ghost 占位 race window:不预占位,streamText 首 chunk 抵达才 INSERT(用户刷新时机早于首 chunk → DB 无 row → 不触发续推死循环) | `tests/_specs/cs-round-028.spec.ts`(vitest 根,跨包) + `ai-cs-demo/src/cs-round-028.spec.ts`(co-located) + `erp-admin-backend/test/cs-round-028.e2e-spec.ts`(jest) | implemented | `ai-cs-demo/src/app/api/chat/route.ts` 删除 :486-504 预占位 else 块 + onChunk 加 fire-and-forget first-chunk INSERT(并入 `lastPatchInFlight` 串行链)+ onFinish/onAbort 加 `await lastPatchInFlight` + Map re-key | AI + you | 2026-08-07   |
