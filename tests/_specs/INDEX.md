...
| cs-round-028                                                  | 🚨 incident: 工单 ↔ 会话脱钩(T-20260807001 在 ERP 详情看不到对话流)— ai-cs sessionKey 类型混淆 / 后端静默回退 / 前端不可见 三段式修复 | `tests/_specs/cs-round-028.spec.ts`(vitest 根,跨包 ai-cs+backend+frontend)                                | draft       | `ai-cs-demo/src/components/chat/ChatView.tsx:207` `sessionKey={activeId ?? ''}` 改为 `sessionKey={activeSessionKey ?? ''}` + page.tsx 透传 `activeSessionKey` + `erp-admin-backend/src/modules/internal/internal.service.ts:494-501` createEscalation sessionKey 未命中改 throw BizException + `erp-admin-frontend/src/components/ConversationPanel.tsx` + `use-conversation.ts` 拉历史失败 setError + 渲染 Alert | AI + you | 2026-08-07   |
